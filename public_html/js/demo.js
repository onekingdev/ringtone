$().ready(function(){
    ...
    $(".checkbox-input").change(function () {
		...
		var ringtoneID = $(this).parent('label').siblings("#ringtoneID").val();
		console.log(ringtoneID);
		if (this.checked) {
			if ($.inArray(ringtoneID, selected_ringtones) < 0) {
				selected_ringtones.push(ringtoneID);
			}
		} else {
			if ($.inArray(ringtoneID, selected_ringtones) >= 0) {
				selected_ringtones.splice($.inArray(ringtoneID, selected_ringtones), 1);
			}
		}
		console.log (selected_ringtones);
		$("#delete-btn").html("<i class='material-icons' style='font-size: 30px;'>delete</i>" + "DELETE (" + selected_ringtones.length + ")");
	});
	
	$("a#delete-btn").click(function(event) {
		if (selected_ringtones.length < 1) {
		    alert ("No Ringtone Selected!");
		    return;
		}
		var items = JSON.stringify(selected_ringtones);
		console.log(items);
		var url = '/ringtone/delete/0.html?'+'ids=' + items;
